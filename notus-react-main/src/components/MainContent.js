...
const MainContent = ({ content, userData }) => {
  ...
  const managerEmail = "hp24@gmail.com"; // Replace with the email of the signed-in user
    ...
    fetch(`http://localhost:5000/api/requirements/${project.projectNumber}`)
      ...
      .then((data) => {
        if (data.status === "ok" && Array.isArray(data.requirements)) {
          ...
          requirementRefs.current = data.requirements.reduce((acc, requirement) => {
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            id="requirementChangeNo"
                            name="requirementChangeNo"
                            value={rmInputs.requirementChangeNo}
                            onChange={handleRmInputChange}
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                          />
                        </div>
